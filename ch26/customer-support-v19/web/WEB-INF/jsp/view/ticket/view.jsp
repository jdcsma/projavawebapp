...
<%--@elvariable id="ticket" type="jun.projavawebapp.site.entities.Ticket"--%>
<%--@elvariable id="comments" type="org.springframework.data.domain.Page<jun.projavawebapp.site.entities.TicketComment>"--%>
            ...
            <c:forEach items="${comments.content}" var="comment">
                ...
                <c:if test="${comment.numberOfAttachments > 0}">
                    ...
                    <c:forEach items="${comment.attachments}" var="attachment"
                               varStatus="status">
                        <c:if test="${!status.first}">, </c:if>
                        <a href="<c:url value="/ticket/attachment/${attachment.id}" />"><c:out
