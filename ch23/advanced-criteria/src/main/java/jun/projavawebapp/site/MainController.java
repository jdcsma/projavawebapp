...
import jun.projavawebapp.site.entities.Gender;
...
public class MainController {
    ...
    public String add(Map<String, Object> model, PersonForm form)
        ...
        person.setBirthDate(new Date(this.formatter.parse(
    ...
    private String find(Map<String, Object> model, SearchForm form,
        ...
        if (form.isIncludeFirstName())
            builder.addCriterion(new Criterion("firstName",
                    Criterion.Operator.EQ, form.getFirstName()));
        if (form.isIncludeMiddleInitial())
            builder.addCriterion(new Criterion("middleInitial",
                    Criterion.Operator.EQ, form.getMiddleInitial()));
        if (form.isIncludeLastName())
            builder.addCriterion(new Criterion("lastName",
                    Criterion.Operator.EQ, form.getLastName()));
        if (form.isIncludeState())
            builder.addCriterion(new Criterion("state",
                    Criterion.Operator.EQ, form.getState()));
        if (form.isIncludeCountry())
            builder.addCriterion(new Criterion("country",
                    Criterion.Operator.EQ, form.getCountry()));
        if (form.isIncludeBirthDate())
            builder.addCriterion(new Criterion("birthDate",
                    Criterion.Operator.EQ, new Date(this.formatter.parse(form.getBirthDate()).getTime())));
        if (form.isIncludeGender())
            builder.addCriterion(new Criterion("gender",
                    Criterion.Operator.EQ, form.getGender()));
        if (form.isIncludeRace())
            builder.addCriterion(new Criterion("race",
                    Criterion.Operator.EQ, form.getRace()));
        if (form.isIncludeEthnicity())
            builder.addCriterion(new Criterion("ethnicity",
                    Criterion.Operator.EQ, form.getEthnicity()));

        model.put("searchForm", form);
        model.put("results", this.personService.searchPeople(builder.build(), pageable));
        return "people/find";
    }
